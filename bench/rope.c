...
static rope_node *iter_at_char_pos(rope *r, size_t char_pos, rope_iter *iter) {
  ...
  while (true) {
    skip = e->nexts[height].skip_chars;
    if (offset > skip) {
      // Go right.
      ...
      // Go down.
      iter->s[height].skip_chars = offset;
...
static rope_node *iter_at_wchar_pos(rope *r, size_t wchar_pos, rope_iter *iter) {
  ...
  while (true) {
    ...
    if (offset > skip) {
      // Go right.
      offset -= skip;
      char_pos += e->nexts[height].skip_chars;
      e = e->nexts[height].node;
    } else {
      // Go down.
      iter->s[height].skip_chars = char_pos;
      iter->s[height].node = e;
  ...
  for (int i = 0; i < r->head.height; i++) {
    iter->s[i].skip_chars = char_pos - iter->s[i].skip_chars;
  }
...
static void update_offset_list(rope *r, rope_iter *iter, size_t num_chars, size_t num_wchars) {
  for (int i = 0; i < r->head.height; i++) {
    iter->s[i].node->nexts[i].skip_chars += num_chars;
...
static void insert_at(rope *r, rope_iter *iter,
  ...
  for (i = 0; i < new_height; i++) {
    ...
    new_node->nexts[i].skip_chars = num_chars + prev_skip->skip_chars - iter->s[i].skip_chars;


    prev_skip->node = new_node;
    prev_skip->skip_chars = iter->s[i].skip_chars;

    // & move the iterator to the end of the newly inserted node.
    iter->s[i].node = new_node;
    iter->s[i].skip_chars = num_chars;
#if ROPE_WCHAR
...
static ROPE_RESULT rope_insert_at_iter(rope *r, rope_node *e, rope_iter *iter, const uint8_t *str) {
  ...
  size_t offset = iter->s[0].skip_chars;
  if (offset) {
...
size_t rope_insert_at_wchar(rope *r, size_t wchar_pos, const uint8_t *str) {
  ...
  size_t pos = iter.s[r->head.height - 1].skip_chars;
  ...
  while (length) {
    ...
    if (removed < num_chars || e == &r->head) {
      ...
      for (i = 0; i < e->height; i++) {
        ...
        iter->s[i].node->nexts[i].skip_chars += e->nexts[i].skip_chars - removed;
#if ROPE_WCHAR
    ...
    for (; i < r->head.height; i++) {
      iter->s[i].node->nexts[i].skip_chars -= removed;
#if ROPE_WCHAR
...
size_t rope_del_at_wchar(rope *r, size_t wchar_pos, size_t wchar_num, size_t *char_len_out) {
  ...
  size_t char_pos = iter.s[r->head.height - 1].skip_chars;

  rope_iter end_iter;
  int h = r->head.height - 1;
  iter_at_wchar_pos(r, iter.s[h].wchar_size + wchar_num, &end_iter);

  size_t char_length = end_iter.s[h].skip_chars - iter.s[h].skip_chars;
...
void _rope_check(rope *r) {
  ...
  assert(skip_over.skip_chars == r->num_chars);
  ...
  for (rope_node *n = &r->head; n != NULL; n = n->nexts[0].node) {
    ...
    assert(count_bytes_in_utf8(n->str, n->nexts[0].skip_chars) == n->num_bytes);
#if ROPE_WCHAR
    assert(count_wchars_in_utf8(n->str, n->nexts[0].skip_chars) == n->nexts[0].wchar_size);
#endif
    for (int i = 0; i < n->height; i++) {
      assert(iter.s[i].node == n);
      assert(iter.s[i].skip_chars == num_chars);
      iter.s[i].node = n->nexts[i].node;
      iter.s[i].skip_chars += n->nexts[i].skip_chars;
#if ROPE_WCHAR
    ...
    num_chars += n->nexts[0].skip_chars;
#if ROPE_WCHAR
...
void _rope_print(rope *r) {
  ...
  printf("HEAD");
  for (int i = 0; i < r->head.height; i++) {
    printf(" |%3zd ", r->head.nexts[i].skip_chars);
  }
  printf("\n");
  ...
  for (rope_node *n = &r->head; n != NULL; n = n->nexts[0].node) {
    printf("%3d:", num++);
    for (int i = 0; i < n->height; i++) {
      printf(" |%3zd ", n->nexts[i].skip_chars);
    }
    printf("        : \"");
